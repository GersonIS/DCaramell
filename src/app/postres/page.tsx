...
const PostresPage = () => {
  const allPostres = useMemo(() => products.filter((p) => p.category === "postres"), []);
  ...
  const filtered = useMemo(() => {
    ...
    let res = allPostres.filter((p) => p.name.toLowerCase().includes(q));
    ...
    return res;
  }, [allPostres, search, minPrice, maxPrice, sort]);
  ...
  return (
    <main className="max-w-7xl mx-auto p-6">
      <header className="flex items-center justify-between mb-6">
        <h1 className="text-3xl font-extrabold">Postres</h1>
