...
const TortasPage = () => {
  const allTortas = useMemo(() => products.filter((p) => p.category === "tortas"), []);
  ...
  const filtered = useMemo(() => {
    ...
    let res = allTortas.filter((p) => p.name.toLowerCase().includes(q));
    ...
    return res;
  }, [allTortas, search, minPrice, maxPrice, sort]);
  ...
  return (
    <main className="max-w-7xl mx-auto p-6">
      <header className="flex items-center justify-between mb-6">
        <h1 className="text-3xl font-extrabold">Tortas</h1>
